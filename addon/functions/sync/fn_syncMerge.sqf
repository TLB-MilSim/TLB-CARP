/*
    USAFDC_fnc_syncMerge

    Fold one crew member's CARP edits into the aircraft's record, and ring the crew.

        [aircraft, changes, senderPayload, authorUid, authorName, authorVersion] call USAFDC_fnc_syncMerge
    ...
    writer of USAFDC_carpRecord: the revision only ever goes up, and each edit lands on
...
// any one player.
_aircraft setVariable ["USAFDC_carpRecord", _newRecord, true];
// The record is the truth; this is only the doorbell, so the crew sees the change now
// instead of on their next sync tick.
["USAFDC_carpRecordChanged", [_aircraft, _newRecord], crew _aircraft] call CBA_fnc_targetEvent;
true
