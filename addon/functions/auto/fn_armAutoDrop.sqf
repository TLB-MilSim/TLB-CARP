...
private _check = [] call TLB_CARP_fnc_validateAutoDrop;
...
if !(_signed > 0) exitWith {
    TLB_CARP_state_autoArmed = false;
    TLB_CARP_state_dropLatched = false;
    TLB_CARP_state_lastSignedRpM = _signed;
    hint "TLB CARP AUTO: RP already passed - fly a fresh approach";
    false
};

TLB_CARP_state_dropLatched = false;
TLB_CARP_state_autoArmed = true;
diag_log format ["[TLB CARP][AUTO] armed sim=%1 carrier=%2 signedRp=%3 cargo=%4",
    ...
    _signed,
    count ([objectParent player] call TLB_CARP_fnc_getLoadedCargo)];
// Rebuild after arming so multi-cargo centering shifts the first-release RP upstream.
private _autoSolution = [objectParent player] call TLB_CARP_fnc_buildWorldSolution;
...
if !(_autoSigned > 0) exitWith {
    TLB_CARP_state_autoArmed = false;
    TLB_CARP_state_lastSignedRpM = _autoSigned;
    hint "TLB CARP AUTO: first-release RP already passed - fly a fresh approach";
    false
};
TLB_CARP_state_solution = _autoSolution;
TLB_CARP_state_lastSignedRpM = _autoSigned;
