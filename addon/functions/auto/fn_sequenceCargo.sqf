...
private _initial = [_carrier] call TLB_CARP_fnc_getLoadedCargo;
...
_carrier setVariable ["TLB_CARP_stickTotal", _countToDrop, false];
_carrier setVariable ["TLB_CARP_stickIndex", -1, false];

for "_index" from 1 to _countToDrop do {
    private _beforeList = [_carrier] call TLB_CARP_fnc_getLoadedCargo;
    ...
    [_carrier, _cargo] call TLB_CARP_fnc_releaseSelected;
    ...
    private _model = if (isNil "TLB_CARP_fnc_getModel") then {createHashMap} else {call TLB_CARP_fnc_getModel};
    ...
    waitUntil {
        sleep 0.01;
        _after = count ([_carrier] call TLB_CARP_fnc_getLoadedCargo);
