if (!USAFDC_state_autoArmed) exitWith {false};
...
if (isNull _vehicle || {!((driver _vehicle) isEqualTo player)}) exitWith {
    [] call USAFDC_fnc_disarmAutoDrop;
    false
};

private _check = [true] call USAFDC_fnc_validateAutoDrop;
...
if (isNull _carrier) exitWith {[] call USAFDC_fnc_disarmAutoDrop; false};
private _manifest = [_carrier] call USAFDC_fnc_getLoadedCargo;
private _available = count _manifest;
private _requested = if (USAFDC_state_cargoCount < 0) then {_available} else {(USAFDC_state_cargoCount min _available) max 1};
...
USAFDC_state_autoDropCommand = [time, diag_tickTime, getPosASL _carrier];
...
_carrier setVariable ["USAFDC_stickTotal", _requested, false];
_carrier setVariable ["USAFDC_stickIndex", -1, false];

diag_log format ["[TLB CARP][AUTO] start sim=%1 real=%2 posASL=%3 velocity=%4 wind=%5 requested=%6 solution=%7", time, diag_tickTime, getPosASL _carrier, velocity _carrier, wind, _requested, USAFDC_state_solution];
[] call USAFDC_fnc_disarmAutoDrop;
if (_requested > 1) then {
    [_carrier, _requested] spawn USAFDC_fnc_sequenceCargo;
    ...
    private _path = [_carrier, _manifest select ((count _manifest) - 1)] call USAFDC_fnc_releaseSelected;
