...
private _profileId = [_vehicle] call TLB_CARP_fnc_resolveAircraftProfile;
...
private _cargoList = [_vehicle] call TLB_CARP_fnc_getLoadedCargo;
...
([_vehicle, _cargo] call TLB_CARP_fnc_releaseModelOffset) params ["_offX", "_offY", "_offZ", "_offSource"];
private _dropPos = [_offX, _offY, _offZ - _bboxTop];
private _releaseModelOffset = [_offX, _offY, _offZ];

private _logKey = format ["TLB_CARP_profileLogged_%1", typeOf _vehicle];
