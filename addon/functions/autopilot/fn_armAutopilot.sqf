...
if (!TLB_CARP_state_guidanceArmed) exitWith {hint "TLB CARP AP: arm guidance first"; false};
if (!TLB_CARP_state_runInLocked) exitWith {hint "TLB CARP AP: lock the run-in first"; false};
...
// refusal immediately followed by "AP ENGAGED" -- and TLB_CARP_state_pathSolution kept
...
private _path = if !(isNil "TLB_CARP_fnc_buildPathSolution")
    then {[_vehicle, _solution] call TLB_CARP_fnc_buildPathSolution}
...
TLB_CARP_state_apArmed = true;
TLB_CARP_state_apState = "INTERCEPT";
...
TLB_CARP_state_apThrottleBaseline = inputAction "HeliThrottlePos";
