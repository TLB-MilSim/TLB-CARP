...
// `[] spawn {uiSleep 0.01; [] call TLB_CARP_fnc_refreshPanel}`, and scheduled code can be
// suspended half way through: with TLB_CARP_state_panelRefreshing raised, so every list on
...
if (canSuspend) exitWith {
    [{[] call TLB_CARP_fnc_refreshPanel}] call CBA_fnc_execNextFrame;
    false
};
...
private _availableCargo = if (isNull _cargoVehicle) then {0} else {
    count ([_cargoVehicle] call TLB_CARP_fnc_getLoadedCargo)
};
...
if (TLB_CARP_state_cargoCount > _availableCargo && {!isNull _cargoVehicle} && {local _cargoVehicle}) then {
...
TLB_CARP_state_panelRefreshing = true;
...
private _dzs = [] call TLB_CARP_fnc_getMissionDZs;
{
    private _i = _dzCtrl lbAdd format ["%1 (%2)", _x # 1, _x # 0];
    _dzCtrl lbSetData [_i, _x # 0];
    if ((TLB_CARP_state_dzName isEqualTo (_x # 1)) || {TLB_CARP_state_dzName isEqualTo (_x # 0)}) then {_selectedDz = _i};
} forEach _dzs;
...
lbClear _modeCtrl;
{
    ...
    if (TLB_CARP_state_mode isEqualTo (_x # 1)) then {_modeCtrl lbSetCurSel _i};
...
lbClear _profileCtrl;
{
    ...
    if (TLB_CARP_state_profileOverride isEqualTo (_x # 1)) then {_profileCtrl lbSetCurSel _i};
} forEach [["AUTO DETECT", ""], ["C-17 OVERRIDE", "c17"], ["C-130 OVERRIDE", "c130"]];

(_display displayCtrl 9305) cbSetChecked TLB_CARP_state_manualWind;
(_display displayCtrl 9306) ctrlSetText str TLB_CARP_state_manualWindMs;
(_display displayCtrl 9307) ctrlSetText str TLB_CARP_state_manualWindFromDeg;
(_display displayCtrl 9306) ctrlEnable TLB_CARP_state_manualWind;
(_display displayCtrl 9307) ctrlEnable TLB_CARP_state_manualWind;
(_display displayCtrl 9308) ctrlEnable TLB_CARP_state_manualWind;
...
// entering.
{
    ...
    [9332, missionNamespace getVariable ["TLB_CARP_state_jumpPlannedStick", 0]],
    [9333, missionNamespace getVariable ["TLB_CARP_state_jumpOpenAglM", 0]]
];

(_display displayCtrl 9320) ctrlSetText str round TLB_CARP_state_targetAglM;
(_display displayCtrl 9321) ctrlSetText str round TLB_CARP_state_targetGroundSpeedKmh;
...
lbClear _cargoCtrl;
{
    ...
    if (TLB_CARP_state_cargoCount isEqualTo (_x # 1)) then {_cargoCtrl lbSetCurSel _i};
} forEach _cargoEntries;
if ((lbCurSel _cargoCtrl) < 0) then {_cargoCtrl lbSetCurSel 0};

(_display displayCtrl 9309) ctrlSetText format ["RUN-IN: %1", if (TLB_CARP_state_runInLocked) then {"LOCKED"} else {"LIVE"}];
(_display displayCtrl 9310) ctrlSetText (if (TLB_CARP_state_guidanceArmed) then {"DISARM GUIDANCE"} else {"ARM GUIDANCE"});
...
// READ WHAT THE BUTTON WILL DO. config.bin's handler toggles on TLB_CARP_state_autoArmed, so
...
if !(isNil "TLB_CARP_fnc_ensurePanelEnhancements") then {[_display] call TLB_CARP_fnc_ensurePanelEnhancements};
