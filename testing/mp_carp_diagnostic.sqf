/*
    TLB CARP -- multiplayer state and cargo diagnostic

    Run this in the debug console on EVERY machine involved: the pilot, the co-pilot, and
    the server if you can reach its console. Sit in the aircraft first.

        [] call USAFDC_diag;

    It prints to systemChat and copies the whole report to the clipboard. Paste all of
    them back and the two reported faults are settled in one pass.

    WHY THIS EXISTS RATHER THAN ANOTHER GUESS

    The deployed PBO in E:\@USAF_CARP_System is v0.8.3 and I verified by unpacking it that
    the cargo manifest, the sync layer and their compile-table registrations are all in
    there and correctly ordered. So the code that should work IS in that build. That
    leaves exactly three possibilities, and this tells them apart:

      1. The machines are not all running that build. Crew sync needs the sync layer on
         BOTH clients -- the publisher and the receiver. If the co-pilot is on an older
         CARP he neither receives the pilot's change nor publishes his own, which is
         symmetric failure and is precisely what was reported. The mod folder I deploy to
         is the one local client; the server and every other player have their own copy.
         LINE 1 of this report settles it.

      2. postInit did not finish on some machine, so the compile table never ran and both
         features are dead together. LINE 2 settles it.

      3. A genuine defect. Then lines 3 onward say which one, because they show the same
         state from both ends.

    THE CARGO QUESTION IS THE SAME SHAPE

    CARP counts four carriage mechanisms: USAF's usaf_cargo, ACE's ace_cargo_loaded,
    vanilla vehicle-in-vehicle, and plain attachTo. This dumps ALL FOUR raw, next to what
    the manifest made of them -- so "CARP cannot see my car" separates into "the car is in
    a source CARP reads and the manifest is broken" from "the car is in none of them".

    That second case is real and likely. A car that Zeus merely PLACED inside the
    fuselage is not loaded by anything: not attached, not vehicle-in-vehicle, not ACE, not
    USAF. It is a car standing on the floor of a flying building. The last line lists
    anything sitting inside or near the aircraft that no mechanism claims, with its
    attachedTo and isVehicleCargo, which is exactly that distinction.
*/
...
USAFDC_diag = {
    ...
    if (isNull _v) then {
        // Run it from outside the aircraft and it still reports on the nearest one, so a
        // loadmaster who has stood up is not stuck.
    ...
    // Access. A client without a CARP Computer (when the mission requires one) neither
    // adopts nor publishes, which from the other seat looks exactly like broken sync.
    ...
    // Where edits are merged. Without CARP on the server each client merges for itself,
    // and edits made in the same instant can overwrite each other.
    format ["server CARP: %1",
        missionNamespace getVariable ["USAFDC_serverVersion", "NOT RUNNING -- clients merge for themselves"]
    ] call _add;

    // The shared record. If the pilot publishes and this stays NONE on the co-pilot, the
    // edit never reached the server or the record never came back. Two seats showing
    // different revs are out of sync; the same rev on both means they hold one record.
    ...
    // The v0.7.0-v0.8.x record. Present only if a crew member is running an old build.
    ...
    // ---- cargo: every mechanism, raw, next to what CARP made of it ------------------
    ...
    // Anything physically in or beside the aircraft that NO mechanism claims. A car Zeus
    // merely placed in the hold shows up here with attachedTo objNull and isVehicleCargo
    // objNull, which is the whole answer: nothing is carrying it, so nothing can drop it.
