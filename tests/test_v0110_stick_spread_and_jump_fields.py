...
class StickSlotTests(unittest.TestCase):
    ...
    def test_the_slots_run_along_the_locked_run_in(self):
        ...
        src = code(BEGIN)
        self.assertIn("sin TLB_CARP_state_runInDeg", src)
        self.assertIn("cos TLB_CARP_state_runInDeg", src)
    ...
    def test_no_run_in_lock_means_no_slot(self):
        ...
        src = code(BEGIN)
        self.assertIn("TLB_CARP_state_runInLocked", src)
    ...
    def test_a_stick_is_closed_by_time(self):
        ...
        src = code(BEGIN)
        self.assertIn('_carrier getVariable ["TLB_CARP_stickLastReleaseS", -1e9]', src)
    ...
    def test_the_total_is_what_was_commanded_never_what_is_in_the_hold(self):
        ...
        src = code(BEGIN)
        self.assertNotIn("TLB_CARP_fnc_getLoadedCargo", src)
        self.assertIn('_total = _carrier getVariable ["TLB_CARP_stickTotal", 1]', src)
    ...
    def test_an_unknown_stick_costs_accuracy_nothing(self):
        ...
        self.assertIn('getVariable ["TLB_CARP_stickTotal", 1]', code(BEGIN))
    ...
    def test_the_commanded_count_is_stamped_where_it_is_known(self):
        ...
        self.assertIn('_carrier setVariable ["TLB_CARP_stickTotal", _requested, false]', trigger)
        self.assertIn('_carrier setVariable ["TLB_CARP_stickIndex", -1, false]', trigger)
        seq = code("addon/functions/auto/fn_sequenceCargo.sqf")
        self.assertIn('_carrier setVariable ["TLB_CARP_stickTotal", _countToDrop, false]', seq)
    ...
    def test_the_stamp_precedes_the_release(self):
        ...
        self.assertLess(src.index("TLB_CARP_stickTotal"), src.index("TLB_CARP_fnc_sequenceCargo"))
        self.assertLess(src.index("TLB_CARP_stickTotal"), src.index("TLB_CARP_fnc_releaseSelected"))

    def test_the_index_advances_within_a_stick(self):
        src = code(BEGIN)
        self.assertIn('_index = (_carrier getVariable ["TLB_CARP_stickIndex", -1]) + 1', src)
        self.assertIn('_carrier setVariable ["TLB_CARP_stickIndex", _index, false]', src)
...
class SlotDeliveryTests(unittest.TestCase):
    ...
    def test_the_tick_hands_it_to_the_steering(self):
        self.assertIn(
            "[_cargo, _dz, _glideMs, _scatterM, _releaseAglM, _engageVzMs, _aimOffset] call TLB_CARP_fnc_steerCargo",
            code(TICK),
        )
    ...
    def test_steer_cargo_takes_it_as_an_argument_and_reads_no_state_for_it(self):
        ...
        self.assertNotIn("TLB_CARP_state_runInDeg", src)
        self.assertNotIn("TLB_CARP_setting_jpadsStickSpacingM", src)
    ...
    def test_the_combined_offset_is_still_picked_once_and_broadcast(self):
        ...
        src = code(STEER)
        self.assertIn('_cargo setVariable ["TLB_CARP_jpadsTargetOffset", _offset, true]', src)
    ...
    def test_the_spacing_setting_exists_and_is_server_forced(self):
        lines = [
            ...
            if "TLB_CARP_setting_jpadsStickSpacingM" in ln and "addSetting" in ln
        ]
...
class JumpPanelFieldTests(unittest.TestCase):
    ...
    def test_the_commit_handler_is_bound_once(self):
        ...
        src = code(ENHANCE)
        self.assertIn('getVariable ["TLB_CARP_commitBound", false]', src)
    ...
    def test_there_is_no_second_altitude_field_for_the_aircraft(self):
        ...
        src = code(ENHANCE)
        self.assertNotIn("TLB_CARP_state_targetAglM", src)


class JumpValueResolutionTests(unittest.TestCase):
    def test_the_crews_stick_size_beats_the_mission_default(self):
        src = code(ARMJUMP)
        planned = src.index("TLB_CARP_state_jumpPlannedStick")
        setting = src.index("TLB_CARP_setting_jumpStickCount")
    ...
    def test_the_crews_opening_altitude_beats_the_mission_default(self):
        src = code(JUMPSOL)
        self.assertIn('missionNamespace getVariable ["TLB_CARP_state_jumpOpenAglM", 0]', src)
        self.assertIn('if (_openAgl <= 0) then {_openAgl = missionNamespace getVariable ["TLB_CARP_setting_jumpOpenAglM", 600]}', src)
    ...
    def test_the_addon_options_survive_as_mission_defaults(self):
        ...
        post = read(POSTINIT)
        self.assertIn("TLB_CARP_setting_jumpOpenAglM", post)
        self.assertIn("TLB_CARP_setting_jumpStickCount", post)

    def test_both_are_crew_intent(self):
        snap = code(SNAPSHOT)
        self.assertIn("TLB_CARP_state_jumpPlannedStick", snap)
        self.assertIn("TLB_CARP_state_jumpOpenAglM", snap)
        apply_src = code(APPLY)
        self.assertIn('"_jumpPlannedStick", "_jumpOpenAglM"', apply_src)
        self.assertIn("TLB_CARP_state_jumpPlannedStick = _jumpPlannedStick;", apply_src)
        self.assertIn("TLB_CARP_state_jumpOpenAglM = _jumpOpenAglM;", apply_src)

    def test_both_are_initialised(self):
        """An unset state variable reads nil, and `nil >= 1` throws rather than being
        false."""
        post = read(POSTINIT)
        self.assertIn("TLB_CARP_state_jumpPlannedStick = 0;", post)
        self.assertIn("TLB_CARP_state_jumpOpenAglM = 0;", post)
