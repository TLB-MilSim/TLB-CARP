...
Mechanism 1 predicts the readout stays gone under canopy: USAFDC_state_jumpRoster is a
...
THE FIX. CARP already allocates its own title layer for the HUD (USAFDC_HUD_LAYER,
cutRsc). Title layers and the hint box are separate subsystems, so a layer CARP allocates
for itself cannot be taken. The readout now goes to USAFDC_JUMP_LAYER with isStructured,
...
class LayerTests(unittest.TestCase):
    ...
    def test_it_has_its_own_layer_separate_from_the_hud(self):
        ...
        src = code(POSTINIT)
        self.assertIn('USAFDC_state_jumpHintLayer = "USAFDC_JUMP_LAYER" call BIS_fnc_rscLayer;', src)
        self.assertIn('USAFDC_state_hudLayer = "USAFDC_HUD_LAYER" call BIS_fnc_rscLayer;', src)
    ...
    def test_the_layer_is_allocated_only_where_there_is_a_screen(self):
        ...
        self.assertLess(src.index('["USAFDC_jumpHint", {'), guard)
        self.assertGreater(src.index('USAFDC_state_jumpHintLayer = "USAFDC_JUMP_LAYER"'), guard)

    def test_the_receiver_tolerates_a_machine_with_no_layer(self):
        src = code(POSTINIT)
        self.assertIn('if (isNil "USAFDC_state_jumpHintLayer") exitWith {};', src)
    ...
    def test_an_empty_string_clears_the_layer(self):
        src = code(POSTINIT)
        self.assertIn('if (_text isEqualTo "") exitWith {USAFDC_state_jumpHintLayer cutText ["", "PLAIN"]};', src)

    def test_the_state_is_initialised_so_isnil_means_something(self):
        self.assertIn("USAFDC_state_jumpHintLayer = nil;", read(POSTINIT))
...
class DisarmClearsEveryScreenTests(unittest.TestCase):
    def test_disarm_broadcasts_a_clear(self):
        ...
        src = code(DISARM)
        self.assertIn('["USAFDC_jumpHint", [""], _clearTargets] call CBA_fnc_targetEvent;', src)

    def test_it_clears_before_the_roster_is_wiped(self):
        """THE ORDERING IS THE POINT. After USAFDC_state_jumpRoster = [] there is nobody
        left to address, and the countdown's last frame would stay on the jumpers'
        screens for the rest of the mission."""
        src = code(DISARM)
        self.assertLess(src.index('["USAFDC_jumpHint", [""], _clearTargets]'),
                        src.index("USAFDC_state_jumpRoster = [];"))
    ...
    def test_it_clears_the_same_audience_the_readout_went_to(self):
        ...
        src = code(DISARM)
        self.assertIn("private _clearRoster = (USAFDC_state_jumpRoster select {alive _x}) + (crew _aircraft);", src)
...
class NothingElseMovedTests(unittest.TestCase):
    def test_the_cue_still_builds_and_targets_exactly_as_before(self):
        ...
        src = code(CUE)
        self.assertIn("private _roster = (USAFDC_state_jumpRoster select {alive _x}) + (crew USAFDC_state_jumpAircraft);", src)
        self.assertIn('["USAFDC_jumpHint", [_this], _targets] call CBA_fnc_targetEvent;', src)

    def test_the_audio_is_untouched(self):
        src = code(CUE)
        self.assertIn('["USAFDC_jumpCue", [_sound], _targets] call CBA_fnc_targetEvent;', src)
    ...
    def test_the_hud_layer_is_not_reused_for_the_readout(self):
        src = code(POSTINIT)
        self.assertNotIn("USAFDC_state_hudLayer cutText [_text", src)
